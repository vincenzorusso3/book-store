...
<script>
	function validate() {
		...
		var ex_name = /^[A-Za-z\s]+$/;
		var ex_username = /^[0-9A-Za-z]+$/;
		var ex_email = /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w+)+$/;
		...
		if (fullname == null || fullname == "") {
			...
			return false;
		} else if (!(name.match(ex_name))) {
			f.name.focus();
			return false;
		} else if (!(username.match(ex_username))) {
			f.username.focus();
			return false;
		} else if (email == null || email == "" || !(email.match(ex_email))) {
			f.email.focus();
			return false;
		}
...
<body>
	...
	<div class="container">
		<div class="row">
			...
			<form name="form" action="ServletRegister" method="post"
